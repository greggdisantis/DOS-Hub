/**
 * Receipt AquaClean Finance Dashboard
...
export function AquacleanReceiptDashboardContent() {
  const colors = useColors();
  ...
  const { data: receipts, isLoading, refetch } = trpc.aquacleanReceipts.list.useQuery(
    Object.keys(filters).length > 0 ? filters : undefined
  );
  const archivedReceipts: any[] = [];
  const isLoadingArchive = false;
  const refetchArchive = useCallback(async () => {}, []);
  const { data: analytics, refetch: refetchAnalytics } = trpc.aquacleanReceipts.analytics.useQuery();
  const { data: usersData } = trpc.users.list.useQuery();
  const deleteReceiptMutation = trpc.aquacleanReceipts.delete.useMutation();
  // archive/unarchive not implemented for AquaClean receipts — use delete instead
  const archiveMutation = { mutateAsync: async (_: any) => {} };
  const unarchiveMutation = { mutateAsync: async (_: any) => {} };
  ...
  const handleExportPDF = useCallback((receipt: any) => {
    router.push({
      pathname: "/modules/aquaclean-receipt-pdf",
